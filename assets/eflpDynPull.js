var dynamicPullevent = new Event('dynamicPullFinished');

    jQuery(function($) {
     /*  ================================================
        OUTFIT PULLING UP IN HERE! (Actually down there)
        ================================================ */

        // pull outfits for Ryan Fit Pants Desktop
        jQuery('.ryanPull').outfitPuller({
            numOfOutfitsToDisplay: 5,
            categoryId: 1072201,
            facetStyleId: 1076470,
            ajaxFailHtml:               '<p class="new-arrivals-outfits-failed">Sorry, there was an error retrieving outfits.</p>',
            ajaxDoneEventName:          'outfitPullerAjaxCallDoneG',
            ajaxFailEventName:          'outfitPullerAjaxCallFailG'
        });
        // pull outfits for the Midi Dress
        jQuery('.midiPull').outfitPuller({
            numOfOutfitsToDisplay: 5,
            categoryId: 1072201,
            facetStyleId: 1076471,
            ajaxFailHtml:               '<p class="new-arrivals-outfits-failed">Sorry, there was an error retrieving outfits.</p>',
            ajaxDoneEventName:          'outfitPullerAjaxCallDoneG',
            ajaxFailEventName:          'outfitPullerAjaxCallFailG'
        });
        // pull outfits for the Plaid Suit
        jQuery('.plaidSuitPull').outfitPuller({
            numOfOutfitsToDisplay: 5,
            categoryId: 1072201,
            facetStyleId: 1076472,
            ajaxFailHtml:               '<p class="new-arrivals-outfits-failed">Sorry, there was an error retrieving outfits.</p>',
            ajaxDoneEventName:          'outfitPullerAjaxCallDoneG',
            ajaxFailEventName:          'outfitPullerAjaxCallFailG'
        });
        // pull outfits for the Belted Mac Trench
        jQuery('.trenchPull').outfitPuller({
            numOfOutfitsToDisplay: 5,
            categoryId: 1072201,
            facetStyleId: 1076473,
            ajaxFailHtml:               '<p class="new-arrivals-outfits-failed">Sorry, there was an error retrieving outfits.</p>',
            ajaxDoneEventName:          'outfitPullerAjaxCallDoneG',
            ajaxFailEventName:          'outfitPullerAjaxCallFailG'
        });
        // pull outfits for the Avery Pant
        jQuery('.averyPull').outfitPuller({
            numOfOutfitsToDisplay: 5,
            categoryId: 1072201,
            facetStyleId: 1076474,
            ajaxFailHtml:               '<p class="new-arrivals-outfits-failed">Sorry, there was an error retrieving outfits.</p>',
            ajaxDoneEventName:          'outfitPullerAjaxCallDoneG',
            ajaxFailEventName:          'outfitPullerAjaxCallFailG'
        });
        ...
        */
        ...
        var pluginName = "outfitPuller",
            defaults = {
                categoryId:                 null,
                facetStyleId:               null,
                productSearchUrl:           '/resources/productSearch/v1/search',
                outfitData:                 null,
                numOfOutfitsToDisplay:      5,
                imgClass:                   'WCD_outfit-image',
                                referralCID: 1072208,//us sale dp
                                referralCIID: 12843316,
                               ...
                               /*'<li class="g-1-2 g-xl-1-4">'+
                                ...
                                '<li class="g-1-2 g-xl-1-4">'+
                                                                ///browse/product.do?cid=1027401&vid=1&pid=187236002
                                                                        //'<a href="/browse/product.do?cid=1027401&vid=1&pid=187236002"'+
...
jQuery.fn.carouselized = function() {
        var thisCar = jQuery(this);
        var browserWidth = window.innerWidth;
        if(browserWidth > 1280) { browserWidth = 1280 };
        var slideWidth = browserWidth * 0.666666667 * 0.25;
        var carContainer = thisCar.closest('.carContainer');
        var leftClick    = thisCar.closest('.dynPullCar').find('.leftClick');
        var rightClick = thisCar.closest('.dynPullCar').find('.rightClick');
        var shopAllTile = thisCar.find('.shopAllImg');
        var slidePos = 0;
        
        thisCar.find('li').css('width',slideWidth);
        thisCar.css('width',slideWidth * 6);
        carContainer.css('height',slideWidth * 100 / 65);
        thisCar.append(shopAllTile);
        
        leftClick.click(function() {
            if(jQuery(this).css('opacity') < 0.2) {
                //do nothing
            } else {            
                rightClick.css('opacity', 1);
                // console.info('clicked left');
                slidePos = slidePos + Math.abs(slideWidth);
                thisCar.css('left',slidePos);
                if(slidePos > -Math.abs(slideWidth)) {
                    jQuery(this).css('opacity', '.15');
                    slidePos = 0;   
                }
            }
        });
        rightClick.click(function() {
            if(jQuery(this).css('opacity') < 0.2) {
                ...
                leftClick.css('opacity',1);
                // console.info('clicked right');
                slidePos = slidePos + -Math.abs(slideWidth);
                thisCar.css('left',slidePos);
                if(slidePos < -Math.abs(slideWidth) * 1) {
                    jQuery(this).css('opacity','.15');
                }
            }
        });
